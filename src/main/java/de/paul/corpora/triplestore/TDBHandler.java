package de.paul.corpora.triplestore;
...
public class TDBHandler extends DocumentIndex {
	...
	@Override
	public FullyExpandedDoc getDocument(String docId) {
		...
		if (title != null && text != null)
			return new FullyExpandedDoc(docId, title, text, annotations);
		else
			return null;
	}

	@Override
	public FullyExpandedDoc getDocument(int docId) {
	...
	public void addExpandedDocument(FullyExpandedDoc doc) {
		ds.end();
	...
	private String createDocTriples(FullyExpandedDoc doc) {
		...
		for (Annotatable ann : annots) {
			...
			// ancestors
			Set<Category> ancAnn = ((FullyExpandedAnnotation) ann)
			...
			Collection<Annotatable> neighAnn = ((FullyExpandedAnnotation) ann)
